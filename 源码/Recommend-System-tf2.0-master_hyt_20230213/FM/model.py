...
class FM_layer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        # inter_part = 0
        # for col_i in range(self.v.shape[0]-1):
        #     for col_j in range(col_i + 1, self.v.shape[0]):
        #         inter_part += tf.reduce_sum(tf.multiply(self.v[col_i, :], self.v[col_j, :])) * tf.multiply(tf.reshape(inputs[:, col_i], [-1, 1]), tf.reshape(inputs[:, col_j], [-1, 1]))
