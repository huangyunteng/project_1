...
def get_feature_param(data_tt, date, date_train_end, ID_OFFSET, X_col, SESS_COUNT=5, DSIN_SESS_MAX_LEN=10):
    ...
    for feat in ["sess_" + str(_x) + "_" + session_feat for _x in range(SESS_COUNT)]:
    ...
    train_idx = data_tt.reset_index(drop=False)[data_tt.index<date_train_end].index.tolist()
...
def get_session1(idx, _data1, _len=10, session_cnt=5):
    ...
    if len(tmp) >= _len + session_cnt:
        ...
        return [tmp[x-_len:x] for x in range(_len, _len + session_cnt)]
    else:
        return [[0 for y in range(_len)] for x in range(_len, _len + session_cnt)]


def DSIN_modeling(data_tt, date, date_train_end, X_col, label_cc="5D", has_trained=False):
    ...
    mms = StandardScaler()
    data_tt[[x for x in X_col if x.split("_")[0]!="dm"]] = mms.fit_transform(data_tt[[x for x in X_col if x.split("_")[0]!="dm"]])

    le = preprocessing.LabelEncoder()
    data_tt["asset_num"] = le.fit_transform(data_tt.asset)
    df_click_eachday = data_tt.groupby("date").apply(lambda x: get_session(x, mode=label_cc))
    tmp_lst = pd.Series(df_click_eachday.index).apply(lambda x: get_session1(x, df_click_eachday, _len=DSIN_SESS_MAX_LEN, session_cnt=DSIN_SESS_COUNT)).tolist()
    df_session = pd.DataFrame(tmp_lst,
                              columns=["sess_" + str(x) + "_asset_num" for x in range(DSIN_SESS_COUNT)],
                              index=df_click_eachday.index)
    df_session["sess_length"] = [DSIN_SESS_MAX_LEN for x in range(len(df_session))]
    data_tt = pd.merge(data_tt.set_index("date"), df_session, left_index=True, right_index=True, how="left")
    params = get_feature_param(data_tt, date, date_train_end, ID_OFFSET, X_col, DSIN_SESS_COUNT, DSIN_SESS_MAX_LEN)
    params["has_trained"] = has_trained
    params["DSIN_SESS_COUNT"] = DSIN_SESS_COUNT
    train_dsin(params)


def time_series_loop(_data, label_cc="5D"):

    src_dict = {
        "engi_feat": (_data.columns[2:-7].tolist(), [label_cc + "_label"]),
        "orig_feat": (_data.columns[2:13].tolist(), [label_cc + "_label"])
    }
     predict_cc = int(label_cc.replace("D",""))

    for date_i in range(2501, len(_data)):
        date = _data.date.drop_duplicates().sort_values().tolist()[date_i]
        date_train_end = _data.date.drop_duplicates().sort_values().tolist()[date_i-predict_cc]
        print("date:{}".format(date))
        for src_key in list(src_dict.keys()):

            X_col, y_col = src_dict[src_key]
            _data_train_valid = _data[_data.date < date_train_end].dropna().reset_index(drop=True)
            _data_test = _data[_data.date == date].dropna().reset_index(drop=True)

            if len(_data_test) == 0:
                continue

            print("DSIN start, src_key:{}".format(src_key))
            DSIN_modeling(_data, date, date_train_end, X_col, label_cc=label_cc, has_trained=False)
...
if __name__ == "__main__":
    ...
    df_data = get_data()
    time_series_loop(df_data, label_cc="3D")