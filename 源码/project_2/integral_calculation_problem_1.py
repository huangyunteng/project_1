...
if __name__ == "__main__":
    ...
    cuts = 2000
    _dict = {}
    # for h in np.arange(-9/40 + (0+9/40)/cuts, 0, (0+9/40)/cuts):
    for h in np.arange(-0.106088, 0, (0 + 9 / 40) / cuts):
        print("h:{}".format(h))
        ...
        y_lst = []
        for _x in np.arange(root_dn, root_up, (root_up-root_dn)/200):
            y = _x * (h - 1 / 8 * _x ** 8 + 1 / 10 * _x ** 5 + 1 / 4 * _x ** 2) ** (1 / 2)
            y_lst.append(y)
        ...
        rs["h"] = _dict.keys()
        rs["P"] = _dict.values()
        rs.to_csv("rs_pro1.csv", index=False)
